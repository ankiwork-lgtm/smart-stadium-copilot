...
type TabId = "dashboard" | "alerts" | "briefing" | "ask";
...
export default function OpsPage() {
  ...
  return (
    ...
    >
      ...
      {/* ------------------------------------------------------------------- */}
      {!isOps && (
        <div className="rounded-xl bg-amber-500/10 border border-amber-500/20 px-4 py-2.5 flex items-center gap-2 text-xs text-amber-300">
          <span>⚠️</span>
          <span>
            You&apos;re viewing Ops Console as <strong>{userContext.role}</strong>.
            Switch to the <strong>Ops</strong> role in the top nav for full access.
          </span>
        </div>
      )}
      ...
      <div className="flex-1 xl:grid xl:grid-cols-[1.1fr_1fr_1fr] gap-4 min-h-0">
        ...
        >
          <div className="rounded-2xl bg-white/[0.025] border border-white/[0.07] overflow-hidden flex flex-col flex-1">
            <div className="px-4 py-3 border-b border-white/[0.06] flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2">
                <span className="text-base">🔔</span>
                <h2 className="text-xs font-semibold text-gray-300">AI Alerts Feed</h2>
              </div>
              <span className="text-[10px] text-gray-600">AI-generated · auto-refresh</span>
            </div>
        ...
        <div
          className={`flex flex-col gap-4 min-h-0
                      ${activeTab === "briefing" || activeTab === "ask" ? "flex" : "hidden xl:flex"}`}
        >
          {/* Briefing Panel */}
          <div
            className={`rounded-2xl bg-white/[0.025] border border-white/[0.07] overflow-hidden shrink-0
                        ${activeTab === "ask" ? "hidden xl:block" : "block"}`}
          >
            <div className="px-4 py-3 border-b border-white/[0.06] flex items-center gap-2">
              <span className="text-base">📋</span>
          ...
          <div
            className={`rounded-2xl bg-white/[0.025] border border-white/[0.07]
                        ...
                        ${activeTab === "briefing" ? "hidden xl:flex" : "flex"}`}
            style={{ minHeight: 320 }}
          >
            <div className="px-4 py-3 border-b border-white/[0.06] flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] bg-indigo-500/10 border border-indigo-500/20 px-2 py-0.5 rounded-full text-indigo-400">
                ops_alert mode
              </span>
            </div>
